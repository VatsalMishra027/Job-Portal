      ...
      <div className='bg-white text-black rounded-lg w-ful'>
        <div className='flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border border-sky-400 rounded-xl'>
          <div className='flex flex-col md:flex-row items-center' >
            <img className='h-24 bg-white rounded-lg p-4 mr-4 max-md:md-4 border' src={assets.company_icon} alt=''/>
          ...
          <div className='w-full lg:w-1/3 mt-8 lg:mt-0 lg:ml-8 space-y-5'>
            ...
            {jobs.filter(job => job._id !== JobData._id && job.companyId._id === JobData.companyId._id).filter(job => {
              const appliedJobIs = new Set(userApplications.map(app => app.JobId && app.JobId._id))
              //return true if the user has not already applied for this jobs
              return !appliedJobIs.has(job._id)
            }).slice(0,4)
